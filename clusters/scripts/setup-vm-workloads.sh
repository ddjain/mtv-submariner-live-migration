...
usage() {
  echo "Usage: $0 --kubeconfig <path> --vm <name> [--namespace <ns>] [--ssh-key <path>] [--ssh-user <user>] [--local-ssh-opts <opts>] [--ssh-ready-timeout SEC]"
  exit 1
}
...
# Unattended virtctl: avoid interactive host-key prompt (same default as run-e2e).
[[ -z "${LOCAL_SSH_OPTS}" ]] && LOCAL_SSH_OPTS="-o StrictHostKeyChecking=accept-new"
...
echo "[1/5] Installing required packages (sqlite, cronie, python3)..."
run_on_vm "sudo dnf install -y sqlite cronie python3 2>&1 | tail -3"
echo "      Done."
echo ""

# --- Step 2: Create data directory ---
echo "[2/5] Creating /data/test directory..."
...
echo '  Crontab:        ' \$(crontab -l 2>/dev/null | head -1)
"

echo ""
echo "============================================"
echo "  Setup complete! All 4 workloads running."
echo "============================================"
