...
Once infrastructure is set up (Phases 1-4), use the automated pipeline under `clusters/` to run migrations with data integrity validation, metrics collection, and chaos testing.

```bash
cd clusters
make run-e2e                    # full pipeline with random VM
make run-e2e LOG_LEVEL=2        # verbose output
make run-e2e-debug              # debug traces
```

See [`clusters/README.md`](clusters/README.md) for full documentation on pipeline steps, logging levels, variables, chaos testing, and reports.
